...
# smartscan.sh
#
# Run SMART short tests on all disks in parallel.
# Automatically elevates to root when required.
#
# Run with:
#
...
if [[ $EUID -ne 0 ]]; then
    ...
    # stdin is occupied by wget | bash, so explicitly attach
    # sudo's input/output to the user's terminal.
    ...
    echo "Restarting SMART Scan as root..."
    echo

    # Re-download the script and execute the complete copy as root.
...
ensure_smartctl() {
    if ! command -v smartctl >/dev/null 2>&1; then
        ...
        apt-get install -y smartmontools >/dev/null || {
            echo "ERROR: Failed to install smartmontools."
            exit 3
        }
    fi
}
